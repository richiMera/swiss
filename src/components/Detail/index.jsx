    ...
    return (
        <motion.div
            ...
            <motion.div onMouseEnter={() => { if (!isMobile) { setIsHovered(true); } }}
                onMouseLeave={() => { if (!isMobile) { setIsHovered(false); } }} style={{ zIndex: '6000', position: isMobile ? 'fixed' : 'absolute', bottom: isMobile ? '50px' : '24px', right: isMobile ? '50%' : '24px', border: '1px solid #2D2D2D', backgroundColor: '#272727', padding: '4px', borderRadius: '50px', display: 'flex', flexDirection: isMobile ? 'row' : 'column', gap: '8px' }} initial={"closed"} animate={item ? "open" : "closed"} variants={variantsYTopIcons}>
